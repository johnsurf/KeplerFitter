function [e, a, Inclination, omega, Omega, Mp, hVec, Latitude, Jacobian] = kepler_vectorized(time, Rpos, Rdot, mu)
...
%function [e p Inclination omega Omega a Jacobian] = kepler(time, Rpos, Rdot)
    ...
    RTransverseSQ = rx.*rx + ry.*ry;
    RadialDistance = sqrt(RTransverseSQ + rz.*rz);
    ...
    Latitude = RadianToDegree*atan2(rz, Rtransverse);
    ...
    Inds   = normN>0;
    Nx     = zeros(size(hx,1),1);
    Ny     = zeros(size(hy,1),1);
    Nz     = zeros(size(hz,1),1);
    
    Nx(Inds) = Ax(Inds)./normN(Inds);                  % 37) Nvec(1) 
    Ny(Inds) = Ay(Inds)./normN(Inds);                  % 38) Nvec(2)
    Nvec   = [Nx, Ny, Nz];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Semi-Major Axis:
    onePe = 1.0 + e;                    % 39) 
    ...
    while any(Omega<0.0)
        Inds  = Omega<0.0;
        Omega(Inds) = Omega(Inds) + twopi;
    end
    while any(Omega>twopi)
        Inds = Omega>twopi; 
        Omega(Inds) = Omega(Inds) - twopi;
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    while any(nuEpoch < 0.0)
        Inds = nuEpoch<0.0;
        nuEpoch(Inds) = nuEpoch(Inds) + twopi;
    end
    while any(nuEpoch > twopi)
        Inds = nuEpoch<twopi;
        nuEpoch(Inds) = nuEpoch(Inds) - twopi;
    end
    % nuEpoch
...
function F = backdiff(f)
    ...
    % from line 62 : if DeltaTime > 0 PTime = DeltaTime; 
      ...
      if any(DeltaTime < 0)
          Inds = DeltaTime<0;
          f(Inds,45) = f(Inds,45) + f(Inds,62);                            % Period  #45
      end
    ...
    % from line 42: rootfac = sqrt(fac);               
      f(Inds,41) = f(Inds,41) + f(Inds,42)*0.5./rootfac(Inds); % fac     #41      
